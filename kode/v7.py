...
GRAYSCALE = True                      # matikan sumbu warna: pintasan palsu hilang
USE_DAE   = False                     # nyalakan HANYA kalau menang di bangku uji
DAE_EPOCHS = 4
# DITERA ke statistik test terukur, bukan ditebak. Setelan v6 lama 3.3x TERLALU
# KUAT (median ketajaman train+degrade 762 vs test 2549). Setelan ini = 0.90x.
# Diuji di bangku korupsi terkalibrasi (n=943, rasio 1.08x vs test asli):
#   augmentasi v5 (RGB)        macro-F1 0.4306  skew 34.68pp
#   grayscale + degrade k=0.30 macro-F1 0.9032  skew  1.59pp
...
class DAE(nn.Module):
    """Penormal masukan. Menghindari 3 kelemahan VAE pada tugas ini:
       1. TANPA downsampling  -> tak ada bottleneck yang memaksa buang detail
       2. keluaran RESIDUAL   -> identitas default; ragu = gambar tak berubah
       3. loss dibobot ke tinta (~17% piksel membawa seluruh identitas kelas)"""
    def __init__(s, c=32):
        super().__init__()
        s.net = nn.Sequential(
        ...
        nn.init.zeros_(s.net[-1].weight); nn.init.zeros_(s.net[-1].bias)  # mulai = identitas
    def forward(s, x): return x + s.net(x)

class PairDS(Dataset):
    """pasangan (rusak, bersih) dari gambar TRAIN untuk melatih DAE"""
    def __init__(s, df, r): s.p = df.path.tolist(); s.r = r
    def __len__(s): return len(s.p)
    def __getitem__(s, i):
        try:
            with Image.open(s.p[i]) as im:
        ...
        clean = im.convert("L").convert("RGB") if GRAYSCALE else im
        dirty = degrade(clean.copy())
        return PLAIN(prep(dirty, s.r, False)), PLAIN(prep(clean, s.r, False))

def train_dae(df, r, epochs=DAE_EPOCHS):
    seed_all(SEED)
    net = DAE().to(DEV)
    opt = torch.optim.AdamW(net.parameters(), lr=2e-3, weight_decay=1e-4)
    ld = DataLoader(PairDS(df, r), batch_size=CFG["batch"], shuffle=True,
                    num_workers=CFG["nw"], drop_last=True)
    sch = torch.optim.lr_scheduler.OneCycleLR(opt, 2e-3,
            total_steps=max(10, epochs*max(1, len(ld))), pct_start=0.3)
    for ep in range(epochs):
        ...
        for xd, xc in ld:
            ...
            out = net(xd)
            w = 1.0 + 4.0*(xc < xc.mean()).float()      # piksel gelap (tinta) dibobot 5x
            loss = (w*(out - xc).abs()).mean()
            loss.backward(); opt.step(); sch.step(); tot += loss.item()
        print(f"    DAE ep{ep} loss={tot/max(1,len(ld)):.5f}")
    net.eval()
    for p_ in net.parameters(): p_.requires_grad_(False)
    return net

class WithDAE(nn.Module):
    """bungkus: DAE beku -> classifier. Dipakai sama persis di train & test."""
    def __init__(s, dae, clf): super().__init__(); s.dae, s.clf = dae, clf
    def forward(s, x): return s.clf(s.dae(x))
...
def train_fold(r, dtr, dva, lr, epochs, seed):
    ...
    if USE_DAE: model = WithDAE(train_dae(dtr, r), model).to(DEV)
...
print(f"\nskew terbesar = {worst:.2f}pp   (v3.1: 11.48pp | v5: 10.00pp, keduanya LB ~0.81)")
if worst < 5.0:
    print("  -> SKEW TURUN DRASTIS: mekanisme pintasan warna memang penyebabnya. Submit.")
else:
    print("  -> SKEW MASIH BESAR: hipotesis pintasan warna GUGUR juga. JANGAN submit dulu,")
    print("     kirim output ini untuk dianalisis ulang.")
